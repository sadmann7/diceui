...
<Steps>
  <Step>
    ...
    This also installs `src/components/ui/direction.tsx`, a thin wrapper around the unified [`radix-ui`](https://www.radix-ui.com/primitives/docs/overview/getting-started) package for RTL support. Individual `@radix-ui/react-*` packages are no longer required.
  </Step>
  <Step>
...
```tsx
import { DataGrid } from "@/components/data-grid/data-grid"; // [!code highlight]
import { DataGridKeyboardShortcuts } from "@/components/data-grid/data-grid-keyboard-shortcuts"; // [!code highlight]
...
Wrap the grid in a `DirectionProvider` and the language direction will be automatically detected. The `direction` component is included when you install the data grid — see the [shadcn Direction docs](https://ui.shadcn.com/docs/components/radix/direction) for the underlying API.
