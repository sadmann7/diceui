...
<ComponentTabs name="presentation-demo" className="h-[520px] p-0" />
...
Load with `readOnly={false}` and place `PresentationSelection` inside `PresentationSlide` to move, resize, and edit text. Undo and redo shortcuts are opt-in via `undoRedoShortcuts`.
...
<ComponentTabs name="presentation-editing-demo" className="h-[520px] p-0" />
