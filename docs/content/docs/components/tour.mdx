...
<Tour.Step target="#element">
  <Tour.Arrow />
  {/* With custom size */}
  <Tour.Arrow width={15} height={8} />
  {/* ... */}
</Tour.Step>
```

The arrow automatically positions itself based on the step's placement. It uses an SVG polygon for perfect alignment and can be customized with `width`, `height`, and `className` props.
