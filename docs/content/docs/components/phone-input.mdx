...
The phone input automatically detects the country from the typed number:

- **Type with `+`**: Detects country immediately (e.g., `+1` → US 🇺🇸)
- **Type digits only**: Detects country after 10+ digits (e.g., `12345678901` → +1 234 567 8901 with US 🇺🇸)

You can also manually select a country from the dropdown before typing.
...
<PhoneInput value={value} onValueChange={setValue}>
  <PhoneInputCountrySelect />
  <PhoneInputField placeholder="12345678901" />
</PhoneInput>
...
The value is stored in E.164 format (e.g., `+12345678901`) for easy backend integration.
