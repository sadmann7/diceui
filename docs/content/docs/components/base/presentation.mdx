...
links:
  ...
  api: /docs/components/base/presentation#api-reference
---
...
`@diceui/pptx` pulls in `@diceui/pptx-core` automatically. The thumbnail rail collapses below 768px behind a toggle, which is why this item also depends on `button` and `use-mobile`.
...
<AutoTypeTable
  path="./types/base/presentation.ts"
