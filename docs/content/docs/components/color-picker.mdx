...
<ComponentTabs name="color-picker-demo" scalePreview />
...
A color picker that renders inline instead of in a popover.
...
- The color swatch includes an `aria-label` that describes the current color value
- The color area and sliders have appropriate `aria-label` attributes for screen readers
- The eye dropper button includes descriptive text for its functionality
