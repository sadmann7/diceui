...
Use the `value` prop on the root to track progress through the timeline. Set it to the index (0-based) of the current step, and all items up to and including that index will be styled with primary color to show completion. The current item (at the exact index) gets an additional ring effect for emphasis.

```tsx
<Timeline.Root value={2}>
  <Timeline.Item>Step 1 - Completed</Timeline.Item>
  <Timeline.Item>Step 2 - Completed</Timeline.Item>
  <Timeline.Item>Step 3 - Current (with ring)</Timeline.Item>
