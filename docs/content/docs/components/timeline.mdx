...
Customize the size of timeline dots using CSS variables. This is useful when you want to add icons or custom content inside the dots.
...
Customize the timeline appearance using CSS variables.
...
<CSSVariablesTable
  data={[
    ...
    {
      title: "--timeline-line-width",
      description: "The width/thickness of the timeline connector line.",
      defaultValue: "0.125rem (2px)",
    },
  ]}
/>

You can customize these variables at different levels:

```tsx
// Customize for all timeline items
<Timeline className="[--timeline-dot-size:2rem]">
  {/* All dots will be 2rem */}
</Timeline>

// Customize for a specific item
<TimelineItem className="[--timeline-dot-size:2rem]">
  <TimelineDot>
    <CustomIcon />
  </TimelineDot>
</TimelineItem>
