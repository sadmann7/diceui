---
title: Direction Provider
description: Provides bidirectional text support (RTL/LTR) across your application.
links:
  api: /docs/utilities/direction-provider#api-reference
  docs: https://radix-ui.com/docs/primitives/utilities/direction-provider
---

## Installation

<CodeTabs>

<TabsList>
  <TabsTrigger value="cli">Command</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>

<TabsContent value="cli">

```package-install
npx shadcn@latest add @diceui/direction-provider
```

</TabsContent>

<TabsContent value="manual">

<Steps className="pt-2">

<Step>Install the following dependencies:</Step>

```package-install
@diceui/direction-provider
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource src="./registry/bases/radix/components/direction-provider.tsx" />

</Steps>

</TabsContent>

</CodeTabs>

## Usage

For data grid and other tablecn components, use the shadcn-style `direction` component at `@/components/ui/direction` instead — it ships automatically with those installs. This utility is for Dice UI components that depend on `@diceui/direction-provider`.

```tsx
import { DirectionProvider } from "@diceui/direction-provider"

export default function App() {
  return (
    <DirectionProvider dir="ltr">
      <YourApp />
    </DirectionProvider>
  )
}
```

## API Reference

### DirectionProvider

Manages direction context for the `useDirection` hook.

<AutoTypeTable
  path="./types/radix/utilities.ts"
  name="DirectionProviderProps"
/>

### useDirection

A hook to access the current direction.

```tsx
import { useDirection } from "@/components/direction-provider"

function Component() {
  const dir = useDirection()
  
  return (
    <Button dir={dir}>
      Do a kickflip
    </Button>
  )
}
```
