...
const TagsInputItemList = React.forwardRef<
  React.ElementRef<typeof TagsInputPrimitive.ItemList>,
  React.ComponentPropsWithoutRef<typeof TagsInputPrimitive.ItemList>
>(({ className, ...props }, ref) => (
  <TagsInputPrimitive.ItemList
    ref={ref}
    className={cn(
...
TagsInputItemList.displayName = TagsInputPrimitive.ItemList.displayName;
...
>(({ className, ...props }, ref) => (
  <TagsInputPrimitive.Item
    ref={ref}
    className={cn(
      "group inline-flex items-center gap-1.5 rounded border bg-transparent py-1 pr-0.5 pl-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 data-[focused]:bg-accent data-[focused]:text-accent-foreground",
      className,
    )}
    {...props}
  >
